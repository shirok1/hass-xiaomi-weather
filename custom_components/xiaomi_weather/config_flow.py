...
CITY_INPUT = vol.All(str, vol.Strip, vol.Length(min=1, max=100))
...
class XiaomiWeatherConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    def _show_source_form(self, errors: dict[str, str]) -> ConfigFlowResult:
        ...
        if self._source == "search":
            schema = {
                vol.Required(CONF_CITY_ID): CITY_INPUT,
                vol.Optional("coordinates"): section(
                    vol.Schema(
                        {
    ...
    async def async_step_confirm(
        ...
        errors: dict[str, str] = {}
        if user_input is not None:
            name = user_input.get("display", {}).get(CONF_NAME, "").strip()
            if name:
                self._resolved[CONF_NAME] = name
            try:
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema(
                {
                    vol.Optional("display"): section(
                        vol.Schema(
                            {
                                vol.Optional(
                                    CONF_NAME, default=self._resolved[CONF_NAME]
                                ): vol.All(str, vol.Strip, vol.Length(max=100)),
                            }
                        ),
                        {"collapsed": not bool(errors)},
                    ),
                    vol.Optional("edit_location", default=False): bool,
                }
            ),
            errors=errors,
        )
