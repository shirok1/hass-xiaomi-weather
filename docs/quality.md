...
| 扩展数据 | 29 个传感器、完整空气质量、日小时 AQI 独立时间轴、预警/台风列表、降水原始序列、昨日/上一时段观测、洗车/运动代码、源时间戳；完整指数通过响应动作提供 |
...
| 实体 | 唯一 ID、service device、has_entity_name、传感器 device/state class；4 个观测/更新时间归类诊断并默认禁用；连同昨日系列、上一时段温度及日/小时预报 AQI 共 11 个默认禁用，沿用 HA 注册机制保留已有启用设置；重载时清理已移除的指数数量实体 |
